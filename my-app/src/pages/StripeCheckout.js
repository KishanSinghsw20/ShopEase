...
function CheckoutForm({ totalAmount, handleOrderSuccess }) {
  ...
  return (
    <form id="payment-form" onSubmit={handleSubmit}>
      ...
      {message && <div id="payment-message" className="text-red-500 mt-4 text-center text-sm">{message}</div>}
    </form>
  );
}
    ...
    <div className="Stripe">
      {clientSecret ? (
        <Elements options={options} stripe={stripePromise}>
          <CheckoutForm totalAmount={totalAmount} handleOrderSuccess={handleOrderSuccess} />
        </Elements>
      ) : (
        <div className="flex flex-col justify-center items-center h-40">
           <div className="text-gray-500">Loading Payment Gateway...</div>
           <p className="text-xs text-gray-400 mt-2 font-mono">Check browser console if stuck</p>
        </div>
      )}
    </div>
  );
